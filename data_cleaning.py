...
def data_cleaning(caminho):
    '''
    Cleaning data from glassdoor_jobs file.
    '''
    df = pd.read_csv(caminho + 'Output\\glassdoor_jobs.csv')    

    df['Per Hour'] = df['Salary Estimate'].apply(lambda x: 1 if 'per hour' in x.lower() else 0) # Identify salary per hour
    df['Employer_Provided'] = df['Salary Estimate'].apply(lambda x: 1 if 'employer provided' in x.lower() else 0) 

    df = df[df['Salary Estimate']!='-1'] # Delete salaries -1
    ...
    # Identify keys words on Job Description field.
